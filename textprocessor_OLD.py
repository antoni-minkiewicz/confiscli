...
class TextProcessor(TextProcessorInterface):
    ...
    def run_full_extraction(self, txt, confis_meta_file, confis_file):
        ...
        self.confis_meta["clause_matcher_output"] = self.clause_matcher.extraction_to_hierarchical_json_from_txt(txt)
        ...
        # self.confis_meta["sentences"] = self.get_sentences_from_txt_no_hitl(txt)
    ...
    def get_confis_clauses_from_txt_no_hitl(self, txt):
        matcher = self.get_clause_matcher_1()
        doc = self.nlp(txt)
        matches = matcher(doc)
        clauses = []
        for match_id, start, end in matches:
            ...
            full_sentence = doc[start - 5:end + 10]

            clauses.append([match_id, string_id, start, end, span.text, full_sentence.text])
            """For verbose"""
            # last_word_of_pattern = doc[end - 1].pos_
            # print(match_id, string_id, start, end, span.text)
            # print(span.text)
        clauses = sorted(clauses, key=itemgetter(2))
        clause_objects = []
        """Refactor this - this doesn't work on a bunch of levels"""
        for clause in clauses:
            str = self.nlp(clause[4])
            ...
            for token in str:
                if (token.pos_ == "PROPN" or token.pos_ == "NOUN"):
                    ...
                    argument_list["conditions"] = token.text
